...
<small><em>
This manuscript
([permalink](https://fmsabatini.github.io/sPlotOpen_Manuscript/v/6eddc6975c39a931219b8a387f7b8dd4c98b8e2f/))
was automatically generated
from [fmsabatini/sPlotOpen_Manuscript@6eddc69](https://github.com/fmsabatini/sPlotOpen_Manuscript/tree/6eddc6975c39a931219b8a387f7b8dd4c98b8e2f)
on October 28, 2020.
</em></small>
...
Note that 2,380 PC1-PC2 grid cells (11.7% of total) had one more confidential vegetation plots (median = 1, mean = 3.4, max = 171) that could not be replaced from the reserve pool.
...
The final dataset that is provided here as open access contains 91,031 vegetation plots from 115 countries and all continents except Antarctica (Figure {@fig:Figure1}) and stems from 103 constitutive datasets (@tbl:Table1). It only contains the species composition of vascular plants, while information on the composition of mosses or lichens was discarded since it was only available for a minority of plots (n = 4,963 and n = 3,045, respectively).
...
A brief description of all the 43 variables contained in the header matrix is provided in @tbl:Table2.  
...
For each record we report both the taxon name as originally contributed by the data custodian (column *'Original_scpecies'*), and the taxon name after taxonomic standardization (column *'Species'*). 
...
We furtehr provide an R function (*'sPlotOpen_citation'*) to create reference lists, based on a selection of plots and\\or datasets.
...
Finally, we harmonized all the taxonomic names from all datasets, based on the sPlot’s taxonomic backbone (Purschke 2017). 
...
As a rule, we recommend sPlto Open users to get in touch with the custodian(s) of the data they are planning to use (custodian names are reported in https://www.idiv.de/sPlot). 
...
## Code Availability  
The R code used to produce sPlot Open from the sPlot 2.1 database is contained in the *sPlotOpen_code* GitHub repository: (https://github.com/fmsabatini/sPlotOpen_Code/). This manuscript was produced using the Manubot workflow (@10.1371/journal.pcbi.1007128). The code for reproducing this manuscript is stored in the *sPlotOpen_manuscript* GitHub repository: (https://github.com/fmsabatini/sPlotOpen_Manuscript).  
...
Milan Chytrý was supported by the Czech Science Foundation (14-36079G, Centre of Excellence Pladias).
...
Mohamed Z. Hatim thanks Kamal Shaltout for supervision, and Joop Schaminée for support and funding from the Prince Bernard Culture Fund Prize for Nature Conservation.
...
|EU-DE-001 |VegMV                                                                                                                                       |   53822|                 5|@doi:10.7809/b-e.00070                |
